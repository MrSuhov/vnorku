"""
Модуль для согласования прилагательных по роду в названиях продуктов
"""
import re
import sys
import os
from typing import Literal, List, Dict, Optional

# Добавляем путь для импорта при прямом запуске
if __name__ == "__main__":
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..'))

from shared.utils.brand_registry import is_brand_text, get_brand_words_in_text

Gender = Literal['m', 'f', 'n', 'unknown']


# ============================================================================
# СЛОВАРЬ РОДА ПРОДУКТОВ
# ============================================================================

PRODUCT_GENDER = {
    # Рыба и морепродукты
    'форель': 'f',
    'семга': 'f',
    'семгу': 'f',
    'горбуша': 'f',
    'горбушу': 'f',
    'кета': 'f',
    'кету': 'f',
    'скумбрия': 'f',
    'скумбрию': 'f',
    'сельдь': 'f',
    'селедка': 'f',
    'треска': 'f',
    'треску': 'f',
    'лосось': 'm',
    'лососось': 'm',
    'карп': 'm',
    'карпа': 'm',
    'окунь': 'm',
    'окуня': 'm',
    'судак': 'm',
    'судака': 'm',
    'тунец': 'm',
    'тунца': 'm',
    'палтус': 'm',
    'палтуса': 'm',
    'сом': 'm',
    'сома': 'm',
    'икра': 'f',
    'икру': 'f',
    'креветка': 'f',
    'креветки': 'f',
    'краб': 'm',
    'краба': 'm',
    'кальмар': 'm',
    'кальмара': 'm',
    'мидия': 'f',
    'мидии': 'f',
    
    # Молочные продукты
    'молоко': 'n',
    'кефир': 'm',
    'кефира': 'm',
    'йогурт': 'm',
    'йогурта': 'm',
    'творог': 'm',
    'творога': 'm',
    'сметана': 'f',
    'сметану': 'f',
    'сливки': 'f',
    'ряженка': 'f',
    'ряженку': 'f',
    'простокваша': 'f',
    'простоквашу': 'f',
    'масло': 'n',
    'сыр': 'm',
    'сыра': 'm',
    'сырок': 'm',
    'сырка': 'm',
    'творожок': 'm',
    'творожка': 'm',
    
    # Мясо и птица
    'курица': 'f',
    'курицу': 'f',
    'куриц': 'f',
    'индейка': 'f',
    'индейку': 'f',
    'утка': 'f',
    'утку': 'f',
    'гусь': 'm',
    'гуся': 'm',
    'говядина': 'f',
    'говядину': 'f',
    'свинина': 'f',
    'свинину': 'f',
    'баранина': 'f',
    'баранину': 'f',
    'телятина': 'f',
    'телятину': 'f',
    'фарш': 'm',
    'фарша': 'm',
    'колбаса': 'f',
    'колбасу': 'f',
    'сосиска': 'f',
    'сосиски': 'f',
    'сарделька': 'f',
    'сардельки': 'f',
    'ветчина': 'f',
    'ветчину': 'f',
    'бекон': 'm',
    'бекона': 'm',
    'грудка': 'f',
    'грудку': 'f',
    'филе': 'n',
    'окорок': 'm',
    'окорока': 'm',
    
    # Овощи
    'картофель': 'm',
    'картофеля': 'm',
    'картошка': 'f',
    'картошку': 'f',
    'морковь': 'f',
    'морковку': 'f',
    'капуста': 'f',
    'капусту': 'f',
    'лук': 'm',
    'лука': 'm',
    'чеснок': 'm',
    'чеснока': 'm',
    'свекла': 'f',
    'свеклу': 'f',
    'огурец': 'm',
    'огурца': 'm',
    'помидор': 'm',
    'помидора': 'm',
    'томат': 'm',
    'томата': 'm',
    'перец': 'm',
    'перца': 'm',
    'баклажан': 'm',
    'баклажана': 'm',
    'кабачок': 'm',
    'кабачка': 'm',
    'тыква': 'f',
    'тыкву': 'f',
    'редис': 'm',
    'редиса': 'm',
    'редька': 'f',
    'редьку': 'f',
    'салат': 'm',
    'салата': 'm',
    'зелень': 'f',
    'петрушка': 'f',
    'петрушку': 'f',
    'укроп': 'm',
    'укропа': 'm',
    'кинза': 'f',
    'кинзу': 'f',
    'базилик': 'm',
    'базилика': 'm',
    'шпинат': 'm',
    'шпината': 'm',
    
    # Фрукты и ягоды
    'яблоко': 'n',
    'груша': 'f',
    'грушу': 'f',
    'банан': 'm',
    'банана': 'm',
    'апельсин': 'm',
    'апельсина': 'm',
    'мандарин': 'm',
    'мандарина': 'm',
    'лимон': 'm',
    'лимона': 'm',
    'грейпфрут': 'm',
    'грейпфрута': 'm',
    'киви': 'n',
    'ананас': 'm',
    'ананаса': 'm',
    'манго': 'n',
    'авокадо': 'n',
    'персик': 'm',
    'персика': 'm',
    'абрикос': 'm',
    'абрикоса': 'm',
    'слива': 'f',
    'сливу': 'f',
    'виноград': 'm',
    'винограда': 'm',
    'арбуз': 'm',
    'арбуза': 'm',
    'дыня': 'f',
    'дыню': 'f',
    'клубника': 'f',
    'клубнику': 'f',
    'земляника': 'f',
    'землянику': 'f',
    'малина': 'f',
    'малину': 'f',
    'черника': 'f',
    'чернику': 'f',
    'голубика': 'f',
    'голубику': 'f',
    'смородина': 'f',
    'смородину': 'f',
    'клюква': 'f',
    'клюкву': 'f',
    'брусника': 'f',
    'бруснику': 'f',
    
    # Напитки
    'сок': 'm',
    'сока': 'm',
    'вода': 'f',
    'воду': 'f',
    'чай': 'm',
    'чая': 'm',
    'кофе': 'm',
    'квас': 'm',
    'кваса': 'm',
    'лимонад': 'm',
    'лимонада': 'm',
    'морс': 'm',
    'морса': 'm',
    'компот': 'm',
    'компота': 'm',
    'напиток': 'm',
    'напитка': 'm',
    
    # Бакалея
    'хлеб': 'm',
    'хлеба': 'm',
    'батон': 'm',
    'батона': 'm',
    'булка': 'f',
    'булку': 'f',
    'лаваш': 'm',
    'лаваша': 'm',
    'крупа': 'f',
    'крупу': 'f',
    'рис': 'm',
    'риса': 'm',
    'гречка': 'f',
    'гречку': 'f',
    'овсянка': 'f',
    'овсянку': 'f',
    'перловка': 'f',
    'перловку': 'f',
    'пшено': 'n',
    'манка': 'f',
    'манку': 'f',
    'макароны': 'm',
    'спагетти': 'n',
    'паста': 'f',
    'пасту': 'f',
    'мука': 'f',
    'муку': 'f',
    'сахар': 'm',
    'сахара': 'm',
    'соль': 'f',
    'уксус': 'm',
    'уксуса': 'm',
    'соус': 'm',
    'соуса': 'm',
    'кетчуп': 'm',
    'кетчупа': 'm',
    'майонез': 'm',
    'майонеза': 'm',
    'горчица': 'f',
    'горчицу': 'f',
    
    # Яйца
    'яйцо': 'n',
    'яйца': 'n',
    'яиц': 'n',
    
    # Сладости
    'шоколад': 'm',
    'шоколада': 'm',
    'конфета': 'f',
    'конфеты': 'f',
    'печенье': 'n',
    'пряник': 'm',
    'пряника': 'm',
    'вафля': 'f',
    'вафли': 'f',
    'торт': 'm',
    'торта': 'm',
    'пирог': 'm',
    'пирога': 'm',
    'кекс': 'm',
    'кекса': 'm',
    'пирожное': 'n',
    'зефир': 'm',
    'зефира': 'm',
    'мармелад': 'm',
    'мармелада': 'm',
    'варенье': 'n',
    'джем': 'm',
    'джема': 'm',
    'мед': 'm',
    'меда': 'm',
}


# ============================================================================
# ТАБЛИЦЫ ОКОНЧАНИЙ ПРИЛАГАТЕЛЬНЫХ
# ============================================================================

ADJECTIVE_ENDINGS = {
    # Женский род → Мужской род
    ('ая', 'm'): 'ый',
    ('яя', 'm'): 'ий',
    ('ья', 'm'): 'ий',
    
    # Женский род → Средний род
    ('ая', 'n'): 'ое',
    ('яя', 'n'): 'ее',
    ('ья', 'n'): 'ее',
    
    # Мужской род → Женский род
    ('ый', 'f'): 'ая',
    ('ий', 'f'): 'ая',
    ('ой', 'f'): 'ая',
    
    # Мужской род → Средний род
    ('ый', 'n'): 'ое',
    ('ий', 'n'): 'ее',
    ('ой', 'n'): 'ое',
    
    # Средний род → Мужской род
    ('ое', 'm'): 'ый',
    ('ее', 'm'): 'ий',
    
    # Средний род → Женский род
    ('ое', 'f'): 'ая',
    ('ее', 'f'): 'яя',
}


# ============================================================================
# ОСНОВНЫЕ ФУНКЦИИ
# ============================================================================

def get_product_gender(product_name: str) -> Gender:
    """
    Определяет род продукта по словарю или эвристикам
    
    Args:
        product_name: Название продукта (может быть несколько слов)
        
    Returns:
        'm' (мужской), 'f' (женский), 'n' (средний), 'unknown'
    """
    if not product_name:
        return 'unknown'
    
    # Приводим к нижнему регистру и берем первое слово
    words = product_name.lower().strip().split()
    if not words:
        return 'unknown'
    
    first_word = words[0]
    
    # Проверяем словарь
    if first_word in PRODUCT_GENDER:
        return PRODUCT_GENDER[first_word]
    
    # Эвристики по окончаниям
    if first_word.endswith(('а', 'я')) and not first_word.endswith(('ия', 'ина')):
        male_exceptions = ['папа', 'дядя', 'юноша', 'слуга']
        if first_word not in male_exceptions:
            return 'f'
    
    if first_word.endswith(('о', 'е', 'мя')):
        return 'n'
    
    if first_word.endswith('ь'):
        return 'f'
    
    return 'm'


def detect_adjective_gender(adjective: str) -> Gender:
    """
    Определяет род прилагательного по окончанию
    
    Args:
        adjective: Прилагательное
        
    Returns:
        'm', 'f', 'n' или 'unknown'
    """
    adj_lower = adjective.lower().strip()
    
    if adj_lower.endswith(('ая', 'яя', 'ья')):
        return 'f'
    
    if adj_lower.endswith(('ый', 'ий', 'ой')):
        return 'm'
    
    if adj_lower.endswith(('ое', 'ее', 'ье')):
        return 'n'
    
    return 'unknown'


def change_adjective_gender(adjective: str, target_gender: Gender) -> str:
    """
    Изменяет окончание прилагательного под целевой род
    
    Args:
        adjective: Исходное прилагательное
        target_gender: Целевой род ('m', 'f', 'n')
        
    Returns:
        Прилагательное с новым окончанием
    """
    if not adjective or target_gender == 'unknown':
        return adjective
    
    adj_lower = adjective.lower()
    current_gender = detect_adjective_gender(adj_lower)
    
    if current_gender == target_gender or current_gender == 'unknown':
        return adjective
    
    # Ищем подходящую замену окончания (от длинного к короткому)
    sorted_endings = sorted(
        ADJECTIVE_ENDINGS.items(),
        key=lambda x: len(x[0][0]),
        reverse=True
    )
    
    for (current_ending, target_g), new_ending in sorted_endings:
        if target_g == target_gender and adj_lower.endswith(current_ending):
            base = adjective[:-len(current_ending)]
            
            # Сохраняем регистр первой буквы
            if adjective[0].isupper():
                return base + new_ending.capitalize() if new_ending[0].islower() else base + new_ending
            else:
                return base + new_ending
    
    return adjective


def detect_adjectives_in_text(text: str) -> List[Dict[str, any]]:
    """
    Находит все прилагательные в тексте (исключая бренды)
    
    Args:
        text: Текст для анализа
        
    Returns:
        Список словарей с информацией о прилагательных
    """
    if not text:
        return []
    
    # Получаем слова брендов в этом тексте
    brand_words = get_brand_words_in_text(text)
    
    words = text.split()
    adjectives = []
    
    for i, word in enumerate(words):
        word_lower = word.lower().strip()
        
        # Пропускаем слова брендов
        if word_lower in brand_words:
            continue
        
        gender = detect_adjective_gender(word)
        if gender != 'unknown':
            adjectives.append({
                'word': word,
                'position': i,
                'gender': gender
            })
    
    return adjectives


def apply_gender_agreement(
    product_name: str,
    common_suffix: str,
    source_gender: Optional[Gender] = None
) -> str:
    """
    Применяет согласование прилагательных по роду
    
    Args:
        product_name: Название продукта (например, "лосось")
        common_suffix: Общая часть с прилагательными (например, "слабосоленая русское море")
        source_gender: Исходный род прилагательных (если известен)
        
    Returns:
        Согласованная строка: "слабосоленый русское море"
    """
    if not common_suffix or not common_suffix.strip():
        return common_suffix
    
    # Определяем род целевого продукта
    target_gender = get_product_gender(product_name)
    
    if target_gender == 'unknown':
        return common_suffix
    
    # Находим все прилагательные в суффиксе (исключая бренды)
    adjectives = detect_adjectives_in_text(common_suffix)
    
    if not adjectives:
        return common_suffix
    
    # Применяем согласование к каждому прилагательному
    words = common_suffix.split()
    result_words = words.copy()
    
    for adj_info in adjectives:
        pos = adj_info['position']
        old_word = adj_info['word']
        new_word = change_adjective_gender(old_word, target_gender)
        
        if new_word != old_word:
            result_words[pos] = new_word
    
    return ' '.join(result_words)


# ============================================================================
# ТЕСТИРОВАНИЕ
# ============================================================================

def test_gender_agreement():
    """Тестирование модуля согласования по роду"""
    print("🧪 Testing Gender Agreement Module")
    print("=" * 80)
    
    # Тест 1: Определение рода продуктов
    print("\n1️⃣ Testing product gender detection:")
    test_products = [
        ('форель', 'f'),
        ('лосось', 'm'),
        ('семга', 'f'),
        ('молоко', 'n'),
        ('кефир', 'm'),
        ('творог', 'm'),
        ('сметана', 'f'),
        ('яйцо', 'n'),
    ]
    
    for product, expected in test_products:
        result = get_product_gender(product)
        status = "✅" if result == expected else "❌"
        print(f"  {status} {product}: {result} (expected {expected})")
    
    # Тест 2: Изменение рода прилагательных
    print("\n2️⃣ Testing adjective gender change:")
    test_adjectives = [
        ('слабосоленая', 'm', 'слабосоленый'),
        ('слабосоленая', 'n', 'слабосоленое'),
        ('свежий', 'f', 'свежая'),
        ('свежий', 'n', 'свежее'),
        ('цельное', 'm', 'цельный'),
        ('цельное', 'f', 'цельная'),
    ]
    
    for adj, target, expected in test_adjectives:
        result = change_adjective_gender(adj, target)
        status = "✅" if result == expected else "❌"
        print(f"  {status} {adj} → {target}: {result} (expected {expected})")
    
    # Тест 3: Полное согласование
    print("\n3️⃣ Testing full gender agreement:")
    test_cases = [
        ('лосось', 'слабосоленая русское море', 'слабосоленый русское море'),
        ('семга', 'слабосоленая русское море', 'слабосоленая русское море'),
        ('кефир', 'цельное 3.2%', 'цельный 3.2%'),
        ('молоко', 'свежий охлажденный', 'свежее охлажденное'),
    ]
    
    for product, suffix, expected in test_cases:
        result = apply_gender_agreement(product, suffix)
        status = "✅" if result == expected else "❌"
        print(f"  {status} {product} + '{suffix}'")
        print(f"      Result:   '{result}'")
        print(f"      Expected: '{expected}'")
        print()
    
    print("=" * 80)
    print("Testing completed!")


if __name__ == "__main__":
    test_gender_agreement()
