...
-- Версия 2: с детализацией доставки и топапов по ЛСД
-- Автор: Korzinka Team
-- Дата: 2025-01-02
...
RETURNS TABLE(
    ...
    -- Поля для детализации доставки и топапов
...
BEGIN
    ...
    LEFT JOIN lsd_stocks ls ON 
        bc.order_item_id = ls.order_item_id 
        AND bc.lsd_config_id = ls.lsd_config_id 
        AND bc.order_id = ls.order_id
        AND bc.price = ls.price
        AND bc.fprice = ls.fprice
