...
-- ИСПРАВЛЕНА: использует lsd_stock_id для устранения дублирования
-- Автор: Korzinka Team
-- Дата: 2025-10-02
...
BEGIN
    ...
    -- ИСПРАВЛЕНИЕ: JOIN по lsd_stock_id вместо составного условия
    RETURN QUERY
    SELECT
...
Result Set 2 (result_type=DELIVERY_DETAILS): детализация доставки и топапов по ЛСД
Result Set 3 (result_type=SUMMARY): сводная информация по корзине с разницей от rank=1

ИСПРАВЛЕНИЕ: Использует bc.lsd_stock_id для JOIN, устраняя дублирование записей';

-- Примеры использования:
-- SELECT * FROM top_baskets(23, 1);
-- SELECT * FROM top_baskets(23, 3);
