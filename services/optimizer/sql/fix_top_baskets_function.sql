-- ==============================================================================
-- ИСПРАВЛЕНИЕ ФУНКЦИИ top_baskets(): добавление delivery_topup в SUMMARY
-- ==============================================================================
...
RETURNS TABLE(
    ...
    topup NUMERIC(10,2),
    delivery_topup JSON  -- НОВОЕ ПОЛЕ
) AS $$
DECLARE
    v_basket_id INTEGER;
...
BEGIN
    ...
    SELECT
        ...
        NULL::NUMERIC(10,2),
        NULL::NUMERIC(10,2),
        ba.delivery_topup  -- НОВОЕ: возвращаем delivery_topup JSON
    FROM basket_analyses ba
    WHERE ba.basket_id = v_basket_id;
...
COMMENT ON FUNCTION top_baskets(INTEGER, INTEGER) IS 'Возвращает детализацию корзины указанного ранга с разделением на ITEMS, DELIVERY_DETAILS и SUMMARY';
