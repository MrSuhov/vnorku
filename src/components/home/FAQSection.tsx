  ...
  const faqs = [
    {
      ...
      answer: 'Мы используем Health-First алгоритм оптимизации, который анализирует калорийность и макронутриенты каждого продукта из нашей базы данных (более 50,000 позиций). Алгоритм перебирает миллионы комбинаций продуктов и выбирает те, которые максимально точно соответствуют вашему плану по питанию. Для расчёта базового обмена веществ используется формула Mifflin-St Jeor (1990) — научно обоснованный метод с точностью ±10% у 82% людей согласно мета-анализу Frankenfield et al. (2005). Эта формула рекомендована Американской диетологической ассоциацией как наиболее точная для современной популяции. Затем мы корректируем калорийность с учётом вашей цели (похудение, набор массы и т.д.).',
    },
    {
